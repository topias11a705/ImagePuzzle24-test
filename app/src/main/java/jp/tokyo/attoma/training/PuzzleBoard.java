...
//切り取る画像矩形の座標配列（24個）の初期化
			img_rect[i]=new Rect((pW*(i%4)),(pH*(i/4)),(pW+pW*(i%4)),(pH+pH*(i/4)));

//正解座標クラス配列（24個）の初期化
			position_rect[i]=new Rect((gazouX+pW2*(i%4)),(gazouY+pH2*(i/4)),(gazouX+pW2*(i%4)+pW2),(gazouY+pH2*(i/4)+pH2));

//画像の現座標レクト配列（24個）の初期化
			sell_rect[i]=new Rect((gazouX+pW2*(i%4)),(gazouY+pH2*(i/4)),(gazouX+pW2*(i%4)+pW2),(gazouY+pH2*(i/4)+pH2));
		}
...
//--------------------------------------------------
	void init(){
		...
		for(int i=0;i<img_rect.length;i++){ list.add(i); }//0～23の数字をlistに入れる
		Collections.shuffle(list);// 0～23の数字の順番をかき混ぜる
		//System.out.println("listの内容 (シャッフル後)");
		//System.out.println(list);// コレクションの内容を表示する

		for(int i=0;i<img_rect.length;i++){ kari_rect[i]=sell_rect[i]; }//System.out.println("入れ替え前");
		for(int i=0;i<img_rect.length;i++){ sell_rect[i]=kari_rect[list.get(i)]; }//System.out.println("入れ替え後");
	}
	//--------------------------------------------------
	//タッチしたパネルを入れ替える
	void changePanel(){
		...
		PuzzleView.firstTouch=PuzzleView.twoTouch=-1;	//タッチしたパネル番号の初期化
		checkmove();
	}
	//----------------------------------------------------------------
//ＨＩＴ数
	void checkmove(){
		int x=0;
		for(int i=0;i<PIECE;i++) if(sell_rect[i].equals(position_rect[i])) x++;
		hitCount=x;
	...
	public void panel_draw(Canvas canvas,int nowImage){
		...
		if(hitCount<PIECE){
			...
			canvas.drawText("HIT:"+hitCount+"/24",(int)(PuzzleView.dw*14),(int)(PuzzleView.dh*460),paint);
